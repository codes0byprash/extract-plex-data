...
def check_for_changes(api_url,cos_filename):
    ...
    try:
        with open (log_file,'r') as f:
            ...
            if change_count> 0 :
                print(f"Fount {change_count} changed records for {cos_filename}")
                return True
            else:
...
def call_plex_api(api_url, cos_filename,headers):
    throttle()
    log_file = f"logs/extraction/{cos_filename}.json"
    if not os.path.exists(log_file):
        return call_first_call(api_url, cos_filename)
    else:
        return incremental_call(api_url, cos_filename)
def call_first_call(api_url, cos_filename):
    try:
...
def incremental_call(api_url, cos_filename):
    try:
        log_file = f"logs/extraction/{cos_filename}.json"
