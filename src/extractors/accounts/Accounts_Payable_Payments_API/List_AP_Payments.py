        ...
        for i in try_range:
            ...
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    if data:
                        all_ap_payments.extend(data)
                        logging.info(f"Fetched {len(data)} payments.")
                    else:
                        logging.info("No payments returned for this interval.")
...
def extract_list_ap_payments(headers, url):
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_range = {
                executor.submit(fetch_payments_for_interval, headers, url, start, end, extra_digit): (start, end)
        ...
        save_extraction_outputs(all_ap_payments, log_dir, resource_name)
        return all_ap_payments

    except Exception as e:
        logging.error(f"Unexpected error in extract_list_ap_payments: {e}")
        return []
