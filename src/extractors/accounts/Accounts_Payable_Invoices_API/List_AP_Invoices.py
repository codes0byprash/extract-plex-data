...
def extract_list_ap_invoices(headers, url):
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_range):
                ...
                try:
                    ...
                    all_ap_invoices.extend(result)
                except Exception as e:
                    logging.error(f"Failed to fetch for interval {start} to {end}: {e}")

        logging.info(f"Total invoices fetched: {len(all_ap_invoices)}")
        save_extraction_metadata(outer_logs_dir, resource_name, len(all_ap_invoices),modified_date_end)

        if not all_ap_invoices:
        ...
        logging.error(f"Unexpected error in extract_list_ap_invoices: {e}")
        return []
